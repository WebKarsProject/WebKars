...
const CardLi = ({ cars }: any) => {
  ...
  return (
    <ListItem listStyleType={'none'} display={'flex'} flexDirection={'column'} gap={'8px'} mb={'44px'}>
      {loading ? (
        <Spinner />
      ) : (
        <>
          <Box display={'flex'} alignItems={'center'} gap={'8px'}>
            ...
            <Text variant={'body-2-500'}>{specificComment?.data.user?.name}</Text>
            <Icon viewBox={'0 0 500 120'} color={'grey_scale.grey3'} />
            <Icon viewBox={'0 0 500 120'} color={'grey_scale.grey3'}>
              <path fill={'currentColor'} d={'M 100, 100 m 30, 0 a 75,75 0 1,0 150,0 a 75,75 0 1,0 -150,0'} />
            </Icon>
